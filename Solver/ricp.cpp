...
RIcp::RIcp(int iters, uint minInlierTh, float maxMahalanobisDist, uint sampleSize)
    : mIterations(iters)
...
bool RIcp::compute(Frame::Ptr pF1, Frame::Ptr pF2, const vector<cv::DMatch>& m12, const bool& updateF2)
{
    mvInliers.clear();
    rmse = 1e6;
...
vector<cv::DMatch> RIcp::sampleMatches(const vector<cv::DMatch>& vMatches)
{
...
Eigen::Matrix4f RIcp::getTransformFromMatches(const vector<cv::DMatch>& vMatches, bool& valid)
{
...
double RIcp::computeInliersAndError(const vector<cv::DMatch>& m12, const Eigen::Matrix4f& transformation4f, std::vector<cv::DMatch>& vInlierMatches)
{
...
double RIcp::errorFunction2(const Eigen::Vector4f& x1, const Eigen::Vector4f& x2, const Eigen::Matrix4d& transformation)
{
...
double RIcp::depthCovariance(double depth)
{
...
double RIcp::depthStdDev(double depth)
{
...
void RIcp::setIterations(int iters) { mIterations = iters; }

void RIcp::setMaxMahalanobisDistance(float dist) { mMaxMahalanobisDistance = dist; }

void RIcp::setSampleSize(uint sampleSize) { mSampleSize = sampleSize; }

void RIcp::setInlierThreshold(uint th) { mMinInlierTh = th; }
